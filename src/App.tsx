...
export default function App() {
  ...
  const handleSubmit = async (text?: string) => {
    ...
    try {
      const apiKey = process.env.GEMINI_API_KEY || "AIzaSyDks_9tIHjxgfGJSxksN-aG3BEO9sZnAlQ";
...
export default function App() {
  ...
  const handleSubmit = async (text?: string) => {
    ...
    try {
      ...
      setChatHistory(prev => {
        const newHistory = [...prev];
        newHistory[newHistory.length - 1] = { ...newHistory[newHistory.length - 1], content: `⚠️ Error: ${errorMessage}\n\nPlease try asking again.` };
        return newHistory;
      });
    } finally {
      setLoading(false);
