              ...
              <main className={`flex-1 backdrop-blur-xl rounded-3xl shadow-2xl border flex flex-col overflow-hidden ${
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 space-y-6 pb-48 sm:pb-56">
                  ...
                  {chatHistory.map((msg, index) => {
                    // Determine bubble styles based on role and dark mode
                    let bubbleClasses = "p-4 sm:p-5 rounded-3xl shadow-md relative group break-words overflow-hidden";
                    if (msg.role === 'user') {
                      bubbleClasses += " bg-gradient-to-br from-indigo-500 to-indigo-600 text-white rounded-tr-sm";
                    } else {
                      // model message
                      if (isDarkMode) {
                        bubbleClasses += " bg-slate-800 border border-slate-700 text-slate-200 rounded-tl-sm";
                      } else {
                        bubbleClasses += " bg-white border border-slate-100 text-slate-800 rounded-tl-sm";
                      }
                    }
